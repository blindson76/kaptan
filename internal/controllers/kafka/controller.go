...
func (c *Controller) configure(sm *stateless.StateMachine) {
    ...
    sm.Configure(StWaitCandidates).
        ...
        PermitIf(TrTimer, StPublish, func(context.Context, ...any) bool { return c.hasEnoughEligible(3) && c.initialWindowElapsed(time.Now()) }).
...
func (c *Controller) needsReplace() bool {
    ...
    for _, id := range c.spec.Members { specSet[id] = true }
...
func (c *Controller) replaceOnce(ctx context.Context) bool {
    ...
    h := map[string]bool{}
    for _, s := range c.health { h[s.ID] = s.Healthy }
    failedIdx := -1
    for i, id := range c.spec.Members {
        if ok, exists := h[id]; exists && !ok {
            failedIdx = i
