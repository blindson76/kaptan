...
func (c *Controller) configure(sm *stateless.StateMachine) {
    sm.Configure(StBoot).
        Permit(TrStart, StWaitCandidates)

    // 1) wait enough candidate reported
    sm.Configure(StWaitCandidates).
        OnEntry(func(ctx context.Context, _ ...any) error {
            log.Printf("[mongo] wait_candidates: waiting for enough eligible candidates")
            c.initialDeadline = time.Time{}
            // If spec already exists, move to wait_health (continue from previous leader)
            if c.loadSpec(ctx) == nil && len(c.spec.Members) > 0 {
                log.Printf("[mongo] existing spec found (members=%v), skipping initial publish", c.spec.Members)
        ...
        PermitIf(TrTimer, StPublish, func(context.Context, ...any) bool {
            // publish only after settle window elapsed
            return c.hasEnoughEligible(3) && c.initialWindowElapsed(time.Now())
        }).
        PermitIf(TrCandidates, StDegraded, func(context.Context, ...any) bool {
            return c.cfg.AllowDegradedSingleMember && !c.hasEnoughEligible(3) && c.hasEnoughEligible(1)
        }).
        PermitIf(TrTimer, StDegraded, func(context.Context, ...any) bool {
            return c.cfg.AllowDegradedSingleMember && !c.hasEnoughEligible(3) && c.hasEnoughEligible(1)
        }).
        Permit(TrPublished, StWaitHealth)

    // 2) deploy initial config (degraded = single member allowed)
    sm.Configure(StDegraded).
        OnEntry(func(ctx context.Context, _ ...any) error {
            log.Printf("[mongo] degraded: publishing single-member spec")
            c.publishSpec(ctx, 1)
            _ = c.sm.FireCtx(ctx, TrPublished)
            return nil
        }).
    ...
    sm.Configure(StPublish).
        OnEntry(func(ctx context.Context, _ ...any) error {
            log.Printf("[mongo] publish: publishing initial spec")
            c.publishSpec(ctx, 3)
            _ = c.sm.FireCtx(ctx, TrPublished)
            return nil
        }).
    ...
    sm.Configure(StWaitHealth).
        ...
        PermitReentry(TrTimer).
        PermitIf(TrHealth, StMonitor, func(context.Context, ...any) bool {
            _ = c.loadSpec(context.Background())
            return c.specAllHealthy()
        }).
        PermitIf(TrTimer, StMonitor, func(context.Context, ...any) bool {
            _ = c.loadSpec(context.Background())
            return c.specAllHealthy()
        })
    ...
    sm.Configure(StMonitor).
        OnEntry(func(ctx context.Context, _ ...any) error {
            log.Printf("[mongo] monitor: monitoring health and failures")
            _ = c.loadSpec(ctx)
            return nil
        }).
        PermitReentry(TrHealth).
        PermitReentry(TrCandidates).
        PermitReentry(TrTimer).
        PermitIf(TrHealth, StReconcile, func(context.Context, ...any) bool {
            _ = c.loadSpec(context.Background())
            return c.needsReplace()
        }).
        PermitIf(TrTimer, StReconcile, func(context.Context, ...any) bool {
            _ = c.loadSpec(context.Background())
            return c.needsReplace()
        })
    ...
    sm.Configure(StReconcile).
        OnEntry(func(ctx context.Context, _ ...any) error {
            log.Printf("[mongo] reconcile: replacing failed member if possible")
            _ = c.loadSpec(ctx)
            changed := c.replaceOnce(ctx)
            if changed {
                _ = c.sm.FireCtx(ctx, TrPublished) // reuse transition to wait_health
            } else {
                // nothing to do, go back to monitor
...
func (c *Controller) maybeStartOrExtendInitialWindow(now time.Time) {
    if c.initialDeadline.IsZero() {
        ...
        log.Printf("[mongo] initial settle window started: %s", c.initialDeadline.Format(time.RFC3339))
        return
    }
    // debounce: push deadline forward on new candidate snapshot
    c.initialDeadline = now.Add(c.cfg.InitialSettleDuration)
}
...
func (c *Controller) hasEnoughEligible(n int) bool {
    return len(c.eligibleCandidates()) >= n
}
...
func (c *Controller) replaceOnce(ctx context.Context) bool {
    ...
    if replacement == "" {
        log.Printf("[mongo] reconcile: no replacement available")
        return false
    }
